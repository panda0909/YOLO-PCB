...
class DetectTab(QWidget):
    ...
    def connect_signals(self):
        ...
        try:
            ...
            self.add_log("檢測器核心模組載入成功")
        except ImportError as e:
            self.detection_worker = None
            self.add_log(f"檢測器核心模組載入失敗: {str(e)}，將使用模擬模式")
    ...
    def on_detection_result(self, result):
        ...
        try:
            ...
            # 如果有圖像顯示器，更新標註
            if hasattr(self, 'image_viewer') and detections:
                annotations = []
                for det in detections:
                    if isinstance(det, dict):
                        # 假設檢測結果包含 bbox 座標和類別信息
                        ...
                        annotations.append({
                            'bbox': [x1, y1, x2, y2],
                            'label': f"{class_name}: {confidence:.2f}",                            'color': (0, 255, 0)  # 綠色框
                        })
    ...
    def get_detection_params(self):
        ...
        params = {
            ...
            'hide_labels': not self.show_labels_checkbox.isChecked(),
            'hide_conf': not self.show_conf_checkbox.isChecked(),
    ...
    def on_detection_error(self, error_message):
        ...
        QMessageBox.critical(self, "檢測錯誤", error_message)
